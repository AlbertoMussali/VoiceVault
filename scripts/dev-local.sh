...
if [[ "$RUN_MIGRATIONS" -eq 1 ]]; then
  ...
  (
    ...
    DATABASE_URL="$DATABASE_URL" uv run --with-requirements requirements.txt alembic upgrade head
  )
fi

log "Launching API, worker, and web dev server..."
start_process "api" "cd \"$ROOT_DIR/apps/api\" && DATABASE_URL=\"$DATABASE_URL\" REDIS_URL=\"$REDIS_URL\" uv run --with-requirements requirements.txt uvicorn app.main:app --host 0.0.0.0 --port \"$API_PORT\" --reload"
start_process "worker" "cd \"$ROOT_DIR/apps/api\" && DATABASE_URL=\"$DATABASE_URL\" REDIS_URL=\"$REDIS_URL\" uv run --with-requirements requirements.txt python -m app.worker"
