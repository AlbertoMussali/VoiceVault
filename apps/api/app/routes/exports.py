...
def _serialize_export_job(job: ExportJob) -> dict[str, object]:
    ...
    if job.status == "completed" and job.artifact_storage_key:
        payload["download_url"] = f"/api/v1/exports/{job.id}/download"
    return payload
...
def create_export_job(
    ...
    try:
        rq_job = enqueue_registered_job("account.export_all_v1", str(export_job.id))
...
def get_export_job(
    ...
    if export_job is None or export_job.user_id != user_id or export_job.export_type != "account_export_all_v1":
...
def download_export_job(
    ...
    try:
        archive_bytes = get_storage_backend().get(export_job.artifact_storage_key)
    ...
    return Response(
        ...
        headers={"Content-Disposition": 'attachment; filename="voicevault-export-all.zip"'},
    )
